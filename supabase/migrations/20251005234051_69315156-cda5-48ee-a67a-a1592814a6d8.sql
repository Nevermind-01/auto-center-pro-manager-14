-- Atualizar função RPC para incluir tipo_entrada ao registrar pagamentos de finalização
...
BEGIN
    ...
    IF v_venda_id IS NULL THEN
        ...
        SELECT 
            v_numero_os,
            v_cliente_id,
            (SELECT nome FROM public.clientes WHERE id = v_cliente_id),
            v_veiculo_id,
            v_mecanico_id,
            v_valor_total,
            v_valor_desconto,
            ...
            v_observacoes,
            CASE 
                WHEN v_tem_carteira THEN 'finalizada-carteira'::venda_status 
    ...
    ELSE
        ...
        SET numero_os = v_numero_os,
            cliente_id = v_cliente_id,
            cliente_nome = (SELECT nome FROM public.clientes WHERE id = v_cliente_id),
        ...
        DELETE FROM public.venda_produtos WHERE venda_id = v_venda_id;
        DELETE FROM public.venda_servicos WHERE venda_id = v_venda_id;
        DELETE FROM public.comissoes_mecanicos WHERE venda_id = v_venda_id;
        DELETE FROM public.os_formas_pagamento WHERE os_id = v_venda_id;
    END IF;

    /* 12) MÚLTIPLAS FORMAS DE PAGAMENTO */
    v_ordem_forma := 0;
    FOR v_forma IN SELECT * FROM jsonb_array_elements(v_formas_pagamento)
    LOOP
        ...
        INSERT INTO public.os_formas_pagamento (
            os_id, forma_pagamento, valor, parcelas, ordem, empresa_id
        ) VALUES (
            v_venda_id,
            v_forma_pg_text::forma_pagamento,
            v_forma_valor,
            v_forma_parcelas,
            v_ordem_forma,
            v_empresa_id
    ...
    IF v_tem_produtos THEN
        ...
        LOOP
            ...
            INSERT INTO public.venda_produtos (
                venda_id, produto_id, produto_nome, categoria_nome,
                quantidade, preco_unitario, preco_total, empresa_id
            ) VALUES (
                v_venda_id,
                (v_produto->>'id')::uuid,
                v_produto->>'nome',
            ...
            INSERT INTO public.movimentacoes (
                produto_id, quantidade, tipo, motivo, os_numero,
                user_id, empresa_id, quantidade_anterior, valor_unitario
            ) VALUES (
                (v_produto->>'id')::uuid,
                -(v_produto->>'quantidade')::integer,
                'saida'::movimento_tipo,
                'Venda OS ' || v_numero_os,
                v_numero_os,
                v_user_id,
                v_empresa_id,
                (SELECT quantidade FROM public.produtos 
                 WHERE id = (v_produto->>'id')::uuid AND empresa_id = v_empresa_id),
                (v_produto->>'valor')::numeric
            );
        END LOOP;
    ...
    IF v_tem_comissao AND v_tem_servicos THEN
        INSERT INTO public.comissoes_mecanicos (
            venda_id, mecanico_id, tipo_calculo,
            percentual, valor_fixo, base_calculo, valor_final,
            observacoes, user_id, empresa_id
        ) VALUES (
            v_venda_id,
            v_mecanico_id,
            v_tipo_calculo,
    ...
    /* 16) CAIXA - MOVIMENTAÇÕES APENAS PARA VALORES NÃO-CARTEIRA */
    IF v_caixa_id IS NOT NULL THEN
        FOR v_forma IN SELECT * FROM jsonb_array_elements(v_formas_pagamento)
        LOOP
            v_forma_pg_text := (v_forma->>'forma')::text;
            ...
            IF v_forma_pg_text <> 'carteira' AND v_forma_valor > 0 THEN
                INSERT INTO public.movimentacoes_caixa (
                    caixa_id, tipo, tipo_origem, referencia_id,
                    forma_pagamento, valor_bruto, valor_liquido,
                    data_hora, criado_por, descricao, empresa_id
                ) VALUES (
                    v_caixa_id, 'entrada'::tipo_movimentacao_caixa,
    ...
    /* 17) REGISTRAR CARTEIRA COMO PENDENTE (NÃO DEBITAR) - COM TIPO_ENTRADA */
    IF v_tem_carteira AND v_valor_carteira > 0 THEN
        INSERT INTO public.pagamentos_os (
            ...
            data_pagamento, usuario_id, empresa_id, observacoes, tipo_entrada
        ) VALUES (
            v_venda_id, 
            ...
            END,
            'finalizacao'  -- MARCAR COMO REGISTRO DE FINALIZAÇÃO
        );
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'vendaId', v_venda_id,
        'numeroOS', v_numero_os,
        'caixaRegistrado', v_caixa_id IS NOT NULL,
