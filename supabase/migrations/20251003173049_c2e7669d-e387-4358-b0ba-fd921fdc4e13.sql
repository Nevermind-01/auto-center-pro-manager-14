-- Atualizar RPC para suportar múltiplas formas de pagamento
...
BEGIN
    ...
    IF payload ? 'formasPagamento' AND payload->'formasPagamento' IS NOT NULL THEN
        ...
        -- Pegar primeira forma como principal para compatibilidade
        v_forma_pagamento := (v_formas_pagamento->0->>'forma')::text;
    ELSE
        -- Fallback para forma única (compatibilidade)
    ...
    IF ABS(v_soma_formas - v_valor_final) > v_tolerancia_calculo THEN
        RAISE EXCEPTION 'Soma das formas de pagamento (%) não confere com valor final (%)', v_soma_formas, v_valor_final;
    END IF;
    ...
    ELSE
        ...
        IF v_venda_id IS NOT NULL THEN
            RETURN jsonb_build_object(
                ...
                'mensagem', 'OS já existia — retorno idempotente'
            );
        END IF;
    END IF;
    ...
    IF v_tem_comissao AND v_tem_servicos THEN
        ...
        IF v_tipo_calculo = 'percentual' THEN
            ...
            IF v_percentual > 100 THEN
                RAISE EXCEPTION 'Percentual de comissão não pode ser maior que 100 por cento';
            END IF;
    ...
    IF v_tem_carteira THEN
        ...
        IF v_saldo_carteira < v_valor_carteira THEN
            RAISE EXCEPTION 'Saldo insuficiente na carteira. Saldo atual: R$ %, Valor necessário: R$ %', 
                v_saldo_carteira, v_valor_carteira;
        END IF;
    END IF;
    ...
    /* 16) MOVIMENTAÇÕES DE CAIXA - UMA POR FORMA */
    IF v_caixa_id IS NOT NULL AND v_caixa_aberto THEN
        ...
        LOOP
            ...
            -- Não criar movimento de caixa para carteira
    ...
    /* 17) PROCESSAR CARTEIRA */
    IF v_tem_carteira AND v_valor_carteira > 0 THEN
        -- Debitar da carteira
        UPDATE public.clientes_carteira
        SET saldo_atual = saldo_atual - v_valor_carteira
        WHERE id = v_carteira_id;
        
        -- Registrar movimentação da carteira
        INSERT INTO public.movimentacoes_carteira (
            cliente_id, tipo, valor, descricao, saldo_anterior, saldo_novo,
            criado_por, empresa_id, os_id, forma_pagamento
        ) VALUES (
            v_cliente_id, 'debito', v_valor_carteira,
            'Pagamento OS ' || v_numero_os || ' - Carteira',
            v_saldo_carteira, v_saldo_carteira - v_valor_carteira,
            v_user_id, v_empresa_id, v_venda_id, 'carteira'
        );
        
        -- Criar registro em pagamentos_os com valor já pago (não-carteira)
        INSERT INTO public.pagamentos_os (
            ...
            v_venda_id, 'carteira'::forma_pagamento, v_valor_nao_carteira, v_valor_carteira,
            NOW(), v_user_id, v_empresa_id,
            'Pagamento inicial - Restante em carteira: R$ ' || v_valor_carteira::text
        );
    END IF;
