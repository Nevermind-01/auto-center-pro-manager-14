-- Implementar plano completo para fortalecer RPC com validações robustas (correção)
...
BEGIN
    ...
    IF ABS(v_valor_total - v_valor_total_calculado) > v_tolerancia_calculo THEN
        RAISE EXCEPTION 'Valor total informado (%) não confere com a soma calculada (%). Diferença: %', 
    ...
    IF ABS(v_valor_final - v_valor_final_calculado) > v_tolerancia_calculo THEN
        RAISE EXCEPTION 'Valor final informado (%) não confere com o cálculo esperado (%). Diferença: %', 
            v_valor_final, v_valor_final_calculado, ABS(v_valor_final - v_valor_final_calculado);
    END IF;
